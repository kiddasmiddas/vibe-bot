"""Integration tests Premium-фичи «Вайб по фото».
...
- Финализация: создание записи в БД и отправка фото в staging-чат.
- Модератор кликает по вайбу → запись completed, profile.own_vibe_id обновлён,
  юзер получает уведомление.
- Модератор отклоняет → запись rejected, юзер получает REJECTED-уведомление.
...
from app.bot.handlers.admin.vibe_by_photo import cb_vibe_by_photo_assign
...
# Делаем staging-чат предсказуемым: всегда отправляем сюда. Меняем глобально
# на время теста через monkeypatch fixture (см. ниже).
...
def _patch_staging_chat(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_cb_vibe_by_photo_start_premium_enters_upload_state(db_session, storage) -> None:
    """Premium-юзер → FSM переходит в vibe_by_photo_upload и предлагается ASK_PHOTOS."""
    ...
    assert data["vbp_photo_file_ids"] == []
...
async def test_on_vibe_by_photo_photo_accumulates(db_session, storage) -> None:
    """Шлём 2 фото — оба попадают в FSM-data, 3-е также принимается, 4-е — отвергается."""
    ...
    await state.update_data(vbp_photo_file_ids=[], vbp_origin="registration")

    def _mk_msg(file_id: str) -> MagicMock:
        msg = _mock_message()
        photo = MagicMock()
        photo.file_id = file_id
        photo.file_size = 100_000  # 100 KB — заведомо в пределах 5 МБ
        msg.photo = [photo, photo]  # photo[-1] is biggest
        return msg

    for fid in ("f1", "f2", "f3"):
        m = _mk_msg(fid)
        await on_vibe_by_photo_photo(m, state, user, db_session)
    ...
    # 4-е — отвергнуто.
    m4 = _mk_msg("f4")
    await on_vibe_by_photo_photo(m4, state, user, db_session)
    m4.answer.assert_awaited_once()
    sent = m4.answer.call_args.args[0]
    assert sent == vbp_texts.PHOTO_MAX_REACHED

    data2 = await state.get_data()
    assert data2["vbp_photo_file_ids"] == ["f1", "f2", "f3"]


# --------------------------- Финализация: создание записи + отправка ---------------------------


@pytest.mark.asyncio
async def test_cb_vibe_by_photo_done_creates_request_and_sends_to_staging(
    db_session, storage
) -> None:
    """`Готово` → создаёт VibeByPhotoRequest со status=pending, шлёт фото в staging."""
    ...
    assert "abc" in req.photo_file_ids and "def" in req.photo_file_ids

    # Фото ушли в staging chat: один без кнопки + один с кнопкой.
    assert bot.send_photo.await_count == 2
    last_call = bot.send_photo.await_args_list[-1]
    assert last_call.kwargs["chat_id"] == _STAGING_CHAT_ID
    assert last_call.kwargs.get("reply_markup") is not None
    # caption на последнем сообщении содержит идентификатор запроса.
    assert str(req.id) in last_call.kwargs["caption"]
...
async def test_admin_pick_vibe_assigns_and_notifies_user(db_session, storage) -> None:
    """Модератор кликает на номер вайба → completed, profile.own_vibe_id обновляется."""
    ...
    # У target юзера должен быть профиль, чтобы own_vibe_id обновился.
    ...
    call_kwargs = bot.send_message.await_args_list[0].kwargs
    assert call_kwargs["chat_id"] == target_user.telegram_id
    assert target_vibe.title in call_kwargs["text"]
...
async def test_admin_reject_marks_rejected_and_notifies_user(db_session, storage) -> None:
    ...
    # Юзеру пришло сообщение об отклонении.
    bot.send_message.assert_awaited()
    sent_kwargs = bot.send_message.await_args_list[0].kwargs
    assert sent_kwargs["chat_id"] == target_user.telegram_id
    assert sent_kwargs["text"] == vbp_texts.REJECTED
...
async def test_cb_edit_vibe_by_photo_start_premium(db_session, storage) -> None:
    ...
    user = await _make_user(db_session, telegram_id=70040, is_premium=True)
    state = _fsm(storage, user_id=user.telegram_id)
...
async def test_cb_edit_vibe_by_photo_done_creates_request(db_session, storage) -> None:
    ...
    user = await _make_user(db_session, telegram_id=70041, username="bob", is_premium=True)
    state = _fsm(storage, user_id=user.telegram_id)
