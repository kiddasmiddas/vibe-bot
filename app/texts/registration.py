...
# Легенда страницы пикера: живые названия вайбов под подписью (номера кнопок
# ↔ коллаж, а названия из справочника — админ переименовал → видно сразу).
VIBE_LEGEND_LINE = "{number} — {title}"
...
# В карточке вайб показываем только название (клиент просил убрать номер,
# 2026-07). Номера живут в пикере регистрации (кнопки-цифры ↔ коллажи страниц);
# шаблон получает и number, и title — лишний kwarg .format() игнорирует.
RENDER_VIBE_NUMBER = "{title}"
