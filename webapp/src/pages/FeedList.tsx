...
import { getFeedPosts, UnauthorizedError } from '../api/client'
...
export function FeedList() {
  ...
  const [items, setItems] = useState<FeedPostPreview[]>([])
  ...
  const loadMore = useCallback(async (currentCursor?: string) => {
    ...
    // Защита от гонки: дочерний вызов с курсором до завершения первой
    // загрузки не должен перезаписать ленту (isInitial → overwrite).
    if (!isFirstLoadDoneRef.current && currentCursor !== undefined) return
    const isInitial = !isFirstLoadDoneRef.current

    setLoadState(isInitial ? 'loading-initial' : 'loading-more')

    try {
      const data = await getFeedPosts(currentCursor)
  ...
  return (
    <main className={styles.page}>
      ...
      {items.length > 0 && (
        <>
          <ul className={styles.grid} aria-label="Лента постов">
            {items.map((post) => (
              <li key={post.id}>
